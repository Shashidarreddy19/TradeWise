-- Database Setup Script for Enterprise Refactored Tariff Data Pipeline
...
-- 2. Category Master (Normalized Parent)
...
-- 3. Category Chapter (Normalized Child - Replaces Comma-Separated Strings)
...
-- 4. Source Master (Extended with Download Controls & Metadata)
...
-- 5. Download History (Enhanced for Incremental Version Detection)
...
-- 9. Validated Layer (hs_validated)
...
-- 10. Master Layer (hs_master with Version Flags & Record Content Hashing)
...
-- 11. Historical Versioning (hs_versions)
...
-- SEED COUNTRY MASTER
INSERT IGNORE INTO country_master (country_code, country_name, customs_territory) VALUES
('IN', 'India', 'IN'),
('US', 'United States', 'US'),
...
-- SEED CATEGORY MASTER & NORMALIZED CHAPTERS
INSERT IGNORE INTO category_master (id, category_name, description) VALUES
...
INSERT IGNORE INTO category_chapter (category_id, chapter) VALUES
(1, '27'),
(2, '71'),
(3, '30'),
(4, '84'),
...
-- SEED SOURCE MASTER (Territory-Deduplicated)
INSERT IGNORE INTO source_master 
(id, customs_territory, country, authority, source_name, download_url, source_type, data_format, nomenclature_type, minimum_code_length, maximum_code_length, preferred_parser, sync_frequency, is_active) VALUES
(1, 'IN', 'India', 'Directorate General of Foreign Trade (DGFT)', 'DGFT ITC(HS) Export Tariff', 'https://dgft.gov.in/', 'BULK_DOWNLOAD', 'CSV', 'ITC_HS', 8, 8, 'CsvHsParser', 'MONTHLY', TRUE),
(2, 'US', 'United States', 'US International Trade Commission (USITC)', 'US Harmonized Tariff Schedule', 'https://hts.usitc.gov/api/search', 'BULK_DOWNLOAD', 'JSON', 'HTS', 8, 10, 'JsonHsParser', 'MONTHLY', TRUE),
(3, 'EU', 'European Union', 'European Commission / TARIC', 'EU Combined Nomenclature (CN) & TARIC', 'https://ec.europa.eu/taxation_customs/dds2/taric/', 'BULK_DOWNLOAD', 'XML', 'CN', 8, 10, 'XmlHsParser', 'MONTHLY', TRUE),
(4, 'GB', 'United Kingdom', 'HM Revenue & Customs (HMRC)', 'UK Trade Tariff Service API', 'https://www.trade-tariff.service.gov.uk/api/v2/commodities', 'API', 'JSON', 'UK_TARIFF', 8, 10, 'UkTradeTariffApiAdapter', 'DAILY', TRUE),
(5, 'GCC', 'GCC Customs Union', 'GCC Customs Council / ZATCA / UAE Customs', 'GCC Integrated Customs Tariff', 'https://zatca.gov.sa/', 'BULK_DOWNLOAD', 'XLSX', 'GCC_TARIFF', 8, 12, 'ExcelHsParser', 'ANNUAL', TRUE),
(6, 'SG', 'Singapore', 'Singapore Customs', 'Singapore Trade Classification & Tariff', 'https://www.customs.gov.sg/', 'BULK_DOWNLOAD', 'XLSX', 'NATIONAL_TARIFF', 8, 8, 'ExcelHsParser', 'ANNUAL', TRUE),
(7, 'BD', 'Bangladesh', 'National Board of Revenue (NBR)', 'Bangladesh Customs Tariff', 'https://customs.gov.bd/', 'BULK_DOWNLOAD', 'PDF', 'NATIONAL_TARIFF', 8, 8, 'PdfHsParser', 'ANNUAL', TRUE),
(8, 'HK', 'Hong Kong', 'Hong Kong Customs & Excise', 'HK Commodity Classification', 'https://www.customs.gov.hk/', 'BULK_DOWNLOAD', 'CSV', 'NATIONAL_TARIFF', 8, 8, 'CsvHsParser', 'ANNUAL', TRUE),
(9, 'CN', 'China', 'General Administration of Customs (GACC)', 'China Customs Tariff Dataset', 'http://www.customs.gov.cn/', 'MANUAL_OFFICIAL_FILE', 'XLSX', 'NATIONAL_TARIFF', 8, 10, 'ExcelHsParser', 'MANUAL', TRUE);

-- 8. Regulation Source Management
...
-- PERFORMANCE INDEXES FOR COMPLIANCE RETRIEVAL ENGINE
CREATE INDEX idx_reg_hs_natcode ON regulation_hs_mapping(national_code);
CREATE INDEX idx_reg_hs_hs6 ON regulation_hs_mapping(hs6);
CREATE INDEX idx_reg_hs_heading ON regulation_hs_mapping(heading);
CREATE INDEX idx_reg_hs_chapter ON regulation_hs_mapping(chapter);
CREATE INDEX idx_reg_hs_regid ON regulation_hs_mapping(regulation_id);
CREATE INDEX idx_reg_master_country ON regulation_master(country);
CREATE INDEX idx_reg_dl_country_src ON regulation_download_history(country, source_id);
CREATE INDEX idx_hs_master_country_natcode ON hs_master(country, national_code);
CREATE INDEX idx_hs_master_country_hs6 ON hs_master(country, hs6);
...
CREATE TABLE IF NOT EXISTS regulation_evidence_audit (
    ...
    validation_status VARCHAR(50) NOT NULL, -- GROUNDED, NOT_FOUND_IN_SOURCE, SUSPICIOUS_TEMPLATED
...
CREATE TABLE IF NOT EXISTS regulation_hs_mapping_audit (
    ...
    heading VARCHAR(50),
    chapter VARCHAR(50),
    hierarchy_level VARCHAR(50) NOT NULL, -- EXACT_NATIONAL_CODE, HS6_SUBHEADING, HEADING_4DIGIT, CHAPTER_2DIGIT
...
CREATE INDEX idx_econ_cty_yr ON country_economic_indicators(country);
...
-- 27. HS REGULATORY COVERAGE AUDIT TABLE
-- Covers every HS code in hs_master × country with full regulatory coverage status.
-- Coverage is computed by matching hs_master codes against regulation_hs_mapping
-- using strict priority: EXACT_NATIONAL_CODE → HS6 → HS4_HEADING → HS2_CHAPTER → NOT_FOUND.
-- No fabricated data. Missing coverage stored as NO_SPECIFIC_HS_REGULATION_FOUND.
...
CREATE INDEX idx_evid_verif_cty ON regulation_hs_evidence_verification(country);
CREATE INDEX idx_evid_verif_status ON regulation_hs_evidence_verification(verification_status);
