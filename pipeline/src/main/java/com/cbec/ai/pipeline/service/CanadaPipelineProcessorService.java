...
@Slf4j
@Service
public class CanadaPipelineProcessorService {

    private final CanadaPdfExtractorService extractorService;
    ...
    public CanadaPipelineProcessorService(
            CanadaPdfExtractorService extractorService,
    ...
    @Data
    @Builder
    public static class CanadaEtlExecutionReport {
    ...
    /**
     * Complete Canadian Customs Tariff PDF ETL Pipeline (01-99-2026-eng.pdf)
     */
    public CanadaEtlExecutionReport processCanadaPipeline(String pdfPath, String version) {
        long startTime = System.currentTimeMillis();
        String ver = version != null ? version : "CANADA_TARIFF_2026";
        String filePath = pdfPath != null ? pdfPath : "01-99-2026-eng.pdf";

        log.info("Starting Canadian Customs Tariff PDF ETL Pipeline for file: {}", filePath);

        PipelineExecutionEntity execution = PipelineExecutionEntity.builder()
                .pipelineName("Canada Customs Tariff PDF ETL Pipeline")
                .country("Canada")
                .source("CBSA Canada Customs Tariff 2026 PDF")
        ...
        // 1. Stage 1: PDF Extraction into hs_raw
        CanadaPdfExtractorService.CanadaPdfExtractionResult extractResult =
                extractorService.extractCanadaTariffPdf(filePath, executionId, ver);

        long totalRawRead = extractResult.getTotalInserted();

        // 2. Read raw records for Canada
        List<HsRawEntity> caRawRecords = hsRawRepository.findByCountry("Canada");
        ...
        for (HsRawEntity raw : caRawRecords) {
            ...
            if (code == null || !code.matches("^\\d{8,10}$")) {
                rejectionsToSave.add(RejectedRecordEntity.builder()
                        ...
                        .reason("INVALID_CANADA_TARIFF_CODE: Code must be 8 or 10 digits")
            ...
            HsValidatedEntity validated = HsValidatedEntity.builder()
                    ...
                    .nomenclatureType("CANADA_TARIFF")
                    .category("PENDING")
        ...
        List<HsValidatedEntity> caValidated = hsValidatedRepository.findAll();
        long totalCategoryMapped = 0;

        for (HsValidatedEntity val : caValidated) {
            if ("Canada".equalsIgnoreCase(val.getCountry()) && "CA".equalsIgnoreCase(val.getCustomsTerritory())) {
        ...
        hsValidatedRepository.saveAll(caValidated);
        ...
        log.info("================================================================================");
        log.info("COMPLETED CANADA CUSTOMS TARIFF PDF ETL PIPELINE");
        log.info("Pages: {}, Chapters: {}, Sections: {}, Extracted: {}, Validated: {}, Master Loaded: {}, Time: {} ms",
                extractResult.getPagesScanned(), extractResult.getChaptersFound(), extractResult.getSectionsFound(),
                totalRawRead, totalValidatedInserted, totalMasterLoaded, duration);
        log.info("================================================================================");

        return CanadaEtlExecutionReport.builder()
                .country("Canada")
                .pdfFileName(extractResult.getPdfFileName())
                .pagesScanned(extractResult.getPagesScanned())
                .chaptersFound(extractResult.getChaptersFound())
                .sectionsFound(extractResult.getSectionsFound())
